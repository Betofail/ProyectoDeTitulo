...
class EnlaceController extends Controller
{
    ...
    public function index()
    {
        ...
        ->select('asignaturas.nombre as asign','seccion_semestres.nrc','docentes.nombre')
        ->where('seccion_semestres.link_encuesta','=','none')->get();

        $asignaturas_con_encuestas = DB::connection('mysql3')->table('seccion_semestres')
        ->join('asignaturas','asignaturas.idAsignatura','=','seccion_semestres.nrc')
        ->join('docentes','docentes.rut','=','seccion_semestres.idDocente')
        ->select('asignaturas.nombre as asign','seccion_semestres.nrc','docentes.nombre')
        ->where('seccion_semestres.link_encuesta','=','limesurvey.test/index.php/'.$list_surveys[0]['sid'])->get();

        return view('enlace',['encuestas' => $list_surveys, 'asignaturas' => $asignaturas,'sid_en' => $list_surveys[0]['sid'],
    ...
    public function tipo_encuesta($id){
        ...
        ->join('docentes','docentes.rut','=','seccion_semestres.idDocente')
        ->select('asignaturas.nombre as asign','seccion_semestres.nrc','docentes.nombre')
        ->where('seccion_semestres.link_encuesta','=','limesurvey.test/index.php/'.$id)->get();

        return view('enlace',['encuestas' => $list_surveys, 'asignaturas' => $asignaturas,'sid_en' => $id,
                    ...
                    foreach ($list_surveys as $key => $value) {
                        if ($value["sid"] == $sid) {
                            DB::connection('mysql3')->table('seccion_semestres')->where([
                                'nrc' => trim($nrc_profesor[0]),
                                'idDocente' => $docente[0]->rut
                            ])->update(['link_encuesta' => 'limesurvey.test/index.php/'.$sid,'fecha_inicio_encuesta' => $value['startdate'],
                    ...
                    foreach ($list_participants as $key => $value) {
                        if ($value == "No survey participants found.") {
                            DB::connection('mysql3')->table('seccion_semestres')->where([
                                'nrc' => trim($nrc_profesor[0]),
                                'idDocente' => $docente[0]->rut
                            ])->update(['link_encuesta' => 'none','fecha_inicio_encuesta' => null,
